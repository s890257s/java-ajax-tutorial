...
> **💡 知識小百科：什麼是 Java 反射 (Reflection)？**
>
> 反射是 Java 提供的一種機制，允許程式在 **執行期間 (Runtime)** 動態地獲取類別的內部資訊（如類別名稱、屬性、方法、建構子）並進行操作，甚至可以存取 `private` 成員。
>
> - **優點**：極高的靈活性。許多框架（如 Spring 的依賴注入、Hibernate、JUnit）都大量使用反射來實現通用功能。
> - **缺點**：
>   - **效能損耗**：因為是在 Runtime 時動態解析，JVM 無法對其進行預編譯優化，速度比直接呼叫程式碼慢。
>   - **類型安全風險**：編譯器無法在編譯時檢查出反射調用的錯誤，必須等到執行時才會發現。
>
> 這也是為什麼 **MapStruct** 被認為優於 **BeanUtils** 的主因：MapStruct 棄用反射，改在編譯時期直接生成純 Java 的 Getter/Setter 代碼。
